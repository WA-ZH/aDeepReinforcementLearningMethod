...
class CVRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state):
        static = reset_state.static
        # shape: (batch, 1, 5)

        node_demand = reset_state.node_demand
        # shape: (batch, problem)
        node_xy_demand = torch.cat((static, node_demand[:, :, None]), dim=2)
        # shape: (batch, problem, 6)  这里把xy坐标和demand拼接起来了

        self.encoded_nodes = self.encoder(node_xy_demand)
...
class CVRP_Encoder(nn.Module):
    ...
    def forward(self, node_xy_demand):
